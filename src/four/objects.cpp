...
bool Plane::intersect( const Ray& r, Hit& h, float tmin ) const {
	...
	bool intersected = false;
	float t = (offset_ - dot(r.origin, normal_))/(dot(r.direction, normal_));
	if (t > tmin && h.t > t) {
...
bool Transform::intersect(const Ray& r, Hit& h, float tmin) const {
	...
	Vec3f origin_space = inverse_ * r.origin;
	Vec3f direction_space = (inverse_ * Vec4f(r.direction,0.0f)).getXYZ();
	

	Ray new_ray = Ray(origin_space, direction_space);
	bool intersect = object_.get()->intersect(new_ray, h, tmin);
	if (intersect) {
		Vec3f normal_space = (inverse_transpose_ * Vec4f(h.normal,0.0f)).getXYZ();
		h.set(h.t,h.material ,normal_space.normalized());
		return true;
	}
	return false; 
}
...
bool Triangle::intersect( const Ray& r, Hit& h, float tmin ) const {
	...
	Vec3f res = M * b;
	float beta = res[0]; float gamma = res[1]; float t = res[2];
	if (beta + gamma < 1 && beta > 0 && gamma > 0 && t > tmin & h.t > t) {
		h.set(t, material_, cross(vertices_[1] - vertices_[0], vertices_[2] - vertices_[1]).normalized());
		return true;
	}
	return false;
}
