...
public:
	...
	static inline FW::Vec2f normalizedImageCoordinateFromPixelCoordinate(const FW::Vec2f& pixel, const FW::Vec2i& imageSize) {
		...
		FW::Vec2f norm_coord = FW::Vec2f();

		norm_coord[0] = (pixel[0] / ((float)imageSize[0] / 2)) - 1;
		norm_coord[1] = (-pixel[1] / ((float)imageSize[1] / 2)) + 1;
...
public:
	...
	virtual Ray generateRay(const FW::Vec2f& point) {
		...
		FW::Vec3f origin_point = FW::Vec3f();
		origin_point = this->center + point[1] * (this->size * this->up) / 2.0f + point[0] * (this->size * this->horizontal) / 2.0f;
		return Ray(origin_point, this->direction);
	}
...
public:
	...
	{	
		...
		float d = 1 / FW::tan(fov_angle/2.0f);
		FW::Vec3f r = point.x * this->horizontal + point.y * this->up + d * this->direction;
		r.normalize();
