		...
		bool shade_back) const
{
	...
	Vec3f pointToEye = (ray.origin - point).normalized();
	Vec3f norma = hit.normal;

	if (shade_back) {
		if (dot(pointToEye, norma) < 0.0f) {
			norma = -1.0f * norma;
		}
	}

	if (dot(dir_to_light, norma) < 0.0f) {
		return answer;
	}

	answer += incident_intensity * FW::clamp(dot(dir_to_light,norma), 0.0f, 1.0f) * diffuse_color(point);

	Vec3f r_vec = -pointToEye - 2 * dot(-pointToEye, norma) * norma;
	r_vec.normalize();
