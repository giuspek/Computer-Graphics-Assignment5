...
void DirectionalLight::getIncidentIllumination(const Vec3f& p,
	...
	dir_to_light = -this->direction_.normalized();
...
void PointLight::getIncidentIllumination(const Vec3f& p,
	...
	Vec3f x = (position_ - p);
	distance = x.length();
	dir_to_light = x.normalized();
	incident_intensity = intensity_ *1.0f / (quadratic_attenuation_ * distance * distance + linear_attenuation_ * distance + constant_attenuation_);
}
